...
public class BossSpectacleListener implements Listener {
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        if (spawningExtra) return; // не реагируем на черепа, которые заспавнили сами
        ...
        spawningExtra = true;
        try {
            shootSkull(wither, from, direction, skull.isCharged());
            ...
            if (random.nextDouble() < doubleChance) {
                ...
                shootSkull(wither, from, spread, skull.isCharged());
            }
        } finally {
            spawningExtra = false;
        }
    ...
    public void onDragonCrystalHeal(EntityRegainHealthEvent event) {
        // Проверка по типу сущности, а не по конкретной причине лечения:
        // не уверен на 100%, что RegainReason.ENDER_CRYSTAL — существующее
        // значение enum в этой версии API (не смог проверить компилятором в
        // песочнице), а дракон в норме и так не лечится никаким другим
        // способом через это событие, так что проверка по EnderDragon даёт
        // тот же практический эффект и гарантированно компилируется.
