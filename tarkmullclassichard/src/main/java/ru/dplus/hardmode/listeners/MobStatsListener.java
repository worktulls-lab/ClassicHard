...
public class MobStatsListener implements Listener {
    ...
    public void onSpawn(CreatureSpawnEvent event) {
        ...
        if (isCreeper) {
            // ванильный фитиль — 30 тиков (1.5 сек) после начала шипения
            double fuseMult = cfg.getDouble("creeper-fuse-multiplier", 0.6);
            Creeper creeper = (Creeper) entity;
            int newFuse = (int) Math.max(1, creeper.getMaxFuseTicks() * fuseMult);
            creeper.setMaxFuseTicks(newFuse);
        }
