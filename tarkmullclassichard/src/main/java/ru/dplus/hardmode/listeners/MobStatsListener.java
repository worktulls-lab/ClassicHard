...
public class MobStatsListener implements Listener {
    ...
    public void onSpawn(CreatureSpawnEvent event) {
        ...
        double speedMult = (entity instanceof Piglin && !(entity instanceof PiglinBrute))
                ? cfg.getDouble("piglin-speed-multiplier", 1.2)
                : cfg.getDouble("mob-speed-multiplier", 1.3);
        ...
        if (entity instanceof Zombie zombie && zombie.isBaby()) {
            ...
            // Если малыш ещё не заспавнился жокеем сам (ванильный шанс ~5%),
            // с доп. шансом довешиваем курицу — превращая часть "обычных"
            // малышей в жокеев, а не добавляя лишних мобов сверху
            if (zombie.getVehicle() == null) {
                double jockeyChance = cfg.getDouble("baby-zombie-jockey-chance", 0.4);
                if (random.nextDouble() < jockeyChance) {
                    Chicken chicken = zombie.getWorld().spawn(zombie.getLocation(), Chicken.class);
        ...
        if (isCreeper) {
            // точное число тиков (20 тиков = 1 сек) вместо множителя — надёжнее
            int fuseTicks = cfg.getInt("creeper-fuse-ticks", 20);
            ((Creeper) entity).setMaxFuseTicks(Math.max(1, fuseTicks));
        }

        if (entity instanceof Spider) {
            // действует и на обычных, и на пещерных пауков (CaveSpider extends Spider)
            double extraSpeed = cfg.getDouble("spider-extra-speed-multiplier", 1.2);
            multiplyAttribute(entity, Attribute.GENERIC_MOVEMENT_SPEED, extraSpeed);
        }
